...
const Dashboard = ({ user }) => {
  ...
  // --- UTILITY: FORMAT TIME (Defined first to prevent scope errors) ---
  const formatTime = (s) => {
  ...
  const generateNewDailyQuests = useCallback(async (userData, daysMissed = 0, oldQuests = []) => {
    ...
    // BACKLOG LOGIC: Migrate yesterday's unfinished to Penalty
    ...
    if (daysMissed > 1) {
        for (let i = 0; i < Math.min(daysMissed, 5); i++) {
            newBacklog.push({ book: "PENALTY", topic: `Missed Daily Training - Gap Day 0${i + 1}`, hours: 2, completed: false, type: 'penalty' });
        }
    }
  ...
  if (loading) return <div className="h-screen bg-black flex items-center justify-center text-system-blue font-system italic animate-pulse text-4xl">SYNCHRONIZING...</div>;

  return (
    <div className="min-h-screen bg-[#050505] text-[#e0e0e0] font-system italic p-4 md:p-10 select-none overflow-x-hidden">
      
      {/* --- STATUS WINDOW / HUNTER CARD --- */}
      {showStatus && (
          ...
          <div className="bg-[#0a0a0c] border-2 border-system-blue w-full max-w-md p-10 relative shadow-[0_0_60px_rgba(0,242,255,0.3)]">
            ...
            <div className="flex justify-between items-start mb-10">
                ...
                <div className="text-right">
                    <p className="text-[10px] text-system-blue font-black uppercase tracking-widest italic">Hunter ID Card</p>
            ...
            <div className="space-y-6 font-mono text-sm">
                ...
                <div className="flex justify-between border-b border-gray-800 pb-2"><span className="text-gray-500">CLASS</span><span className="text-system-accent font-black uppercase italic">{playerData.studentType}</span></div>
            </div>
            <button onClick={() => window.print()} className="w-full mt-10 bg-system-blue/10 border border-system-blue text-system-blue py-3 font-black uppercase text-[10px] tracking-widest hover:bg-system-blue hover:text-black transition-all flex items-center justify-center gap-2"><Download size={14}/> Generate ID Card</button>
          </div>
        </div>
      )}
      ...
      {showHelp && (
        <div className="fixed inset-0 z-[120] bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#0a0a0c] border-2 border-system-purple w-full max-w-lg p-10 relative">
            ...
            <h2 className="text-3xl font-black text-system-purple uppercase mb-10 italic border-b border-gray-900 pb-4 flex items-center gap-4"><Info size={28}/> System Intelligence</h2>
            <div className="space-y-6 text-[10px] text-gray-400 font-mono leading-relaxed uppercase tracking-tighter italic">
              <p><span className="text-system-blue font-black mr-2">STAMINA:</span> Syncs with XP progress. 5 Clearance = 1 LVL UP.</p>
              <p><span className="text-system-purple font-black mr-2">MANA:</span> Live output of your study time vs your daily goal.</p>
              <p><span className="text-red-500 font-black mr-2">PENALTY:</span> Daily missed training. Must be cleared to regain rank.</p>
            </div>
          </div>
        </div>
      )}
      ...
      {isRaidActive && (
        <div className="fixed inset-0 z-[110] bg-black/98 flex flex-col items-center justify-center border-4 border-red-900/10 backdrop-blur-xl">
          ...
          <button onClick={() => clearQuest()} className="px-24 py-5 bg-system-blue text-black font-black uppercase shadow-[0_0_40px_#00f2ff] hover:scale-110 transition-all tracking-[0.3em] italic">Mark objective complete</button>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center border-b-2 border-gray-900 pb-10 mb-12 gap-8 relative">
        ...
        <div className="bg-[#0a0a0a] border-2 border-gray-800 p-5 flex items-center gap-12 shadow-2xl relative overflow-hidden group min-w-[380px]">
            ...
            <div className="relative z-10 flex items-center gap-5">
                ...
                <div><p className="text-[9px] text-gray-500 font-black uppercase tracking-[0.4em] mb-1 italic">Mana extraction</p><div className="text-4xl font-black text-white tracking-tighter font-mono leading-none">{formatTime(isTimerActive ? sessionSeconds : totalSecondsToday)}</div></div>
            </div>
            <button onClick={toggleStopwatch} className={`relative z-10 p-5 rounded-full transition-all active:scale-90 ${isTimerActive ? 'bg-red-900/20 text-red-500 border-2 border-red-500 shadow-[0_0_20px_#ff000033]' : 'bg-system-blue/20 text-system-blue border-2 border-system-blue shadow-[0_0_30px_#00f2ff44]'}`}>{isTimerActive ? <Pause size={30} fill="currentColor" /> : <Play size={30} fill="currentColor" />}</button>
        </div>

        <div className="flex items-center gap-6">
          <div className="text-right hidden xl:block"><p className="text-[8px] text-gray-600 font-black uppercase tracking-tighter mb-1 italic">System Stability</p><p className="text-[11px] font-mono text-system-accent tracking-widest font-black uppercase animate-pulse italic uppercase">Optimized</p></div>
              ...
              <div key={i} className={`group relative border-2 transition-all duration-500 ${quest.completed ? 'bg-black/40 border-gray-900 opacity-40 shadow-none' : 'bg-[#0a0a0a] border-gray-800 hover:border-system-blue shadow-2xl'} p-10 overflow-hidden`}>
                <div className="flex justify-between items-center relative z-10">
                  <div className="max-w-[75%]">
                    <p className={`text-[11px] font-black uppercase tracking-[0.6em] mb-3 italic ${quest.type ? 'text-red-500' : 'text-system-blue'}`}>{quest.type ? quest.book : `Grimoire: ${quest.book?.split(' - ')[0] || quest.book}`}</p>
